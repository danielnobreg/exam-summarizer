...
export default function Xray({ user, onLogout, onNavigate }) {
  ...
  const [position, setPosition] = useState('PA (Em pé)');
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      const result = await analysisService.analyzeXray(promptText, base64Images);
    ...
    <div className="min-h-screen bg-gray-50 font-manrope pt-20 flex flex-col">
      ...
      <div className="flex-1 max-w-4xl mx-auto px-4 py-8 w-full">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-4xl font-extrabold text-gray-900 mb-3 tracking-tight">Análise de Raio-X</h1>
          <p className="text-lg text-gray-500 max-w-2xl mx-auto leading-relaxed">
            Faça upload das imagens do Raio-X de Tórax e informe os dados clínicos para uma avaliação precisa com IA.
          </p>
        </div>
        ...
        <div className="bg-white rounded-[2rem] shadow-xl p-8 border border-gray-100 relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">Ventilação</label>
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="radio" value="Espontânea" checked={ventilation === 'Espontânea'} onChange={(e) => setVentilation(e.target.value)} className="text-blue-600 focus:ring-blue-500" />
                  <span className="text-sm text-gray-700">Espontânea</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="radio" value="Mecânica" checked={ventilation === 'Mecânica'} onChange={(e) => setVentilation(e.target.value)} className="text-blue-600 focus:ring-blue-500" />
            ...
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">Posição do Paciente</label>
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="radio" value="PA (Em pé)" checked={position === 'PA (Em pé)'} onChange={(e) => setPosition(e.target.value)} className="text-blue-600 focus:ring-blue-500" />
                  <span className="text-sm text-gray-700">PA (Em pé)</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="radio" value="AP (No leito)" checked={position === 'AP (No leito)'} onChange={(e) => setPosition(e.target.value)} className="text-blue-600 focus:ring-blue-500" />
                  <span className="text-sm text-gray-700">AP (No leito)</span>
                </label>
              </div>
            </div>
          </div>
          ...
          <div className="mb-8">
            <label className="block text-sm font-bold text-gray-700 mb-2">Observações Clínicas (Opcional)</label>
            <textarea 
              value={obs}
              onChange={(e) => setObs(e.target.value)}
              placeholder="Ex: Paciente com febre há 3 dias, tosse produtiva. Tem dreno à direita."
          ...
          {/* Área de Upload */}
          <div className="mb-8">
            <label className="block text-sm font-bold text-gray-700 mb-3 uppercase tracking-wide">Imagens do Raio-X (Até 2)</label>
            <label 
              htmlFor="file-upload" 
              className={`group flex flex-col items-center justify-center w-full h-40 border-2 border-dashed rounded-2xl cursor-pointer transition-all duration-300 ${
                isDragging ? 'border-blue-400 bg-blue-50 scale-[1.02]' : 'border-gray-200 bg-gray-50/50 hover:bg-white hover:border-blue-300 hover:shadow-md'
              }`}
            ...
            >
              <div className={`transition-transform duration-300 ${isDragging ? 'scale-110' : 'group-hover:scale-110'}`}>
                <div className={`w-14 h-14 rounded-full flex items-center justify-center mb-3 transition-colors ${isDragging ? 'bg-blue-100' : 'bg-white shadow-sm group-hover:bg-blue-50'}`}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className={`transition-colors ${isDragging ? 'text-blue-600' : 'text-gray-400 group-hover:text-blue-500'}`}>
                    <rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect>
                    <circle cx="8.5" cy="8.5" r="1.5"></circle>
                    <polyline points="21 15 16 10 5 21"></polyline>
                    </svg>
              ...
              <p className="text-sm text-gray-700 mb-1 font-medium">
                {files.length >= 2 ? (
                  ...
                  <><span className="text-blue-600 font-bold border-b border-blue-200 hover:border-blue-600 transition-colors">Clique para enviar</span> ou arraste imagens</>
                )}
              </p>
          ...
          {files.length > 0 && (
            <div className="mb-6 grid gap-3 grid-cols-1 md:grid-cols-2">
              {files.map((file, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 bg-green-50 border border-green-100 rounded-xl animate-fadeIn">
            ...
            <div className="mt-10 pt-8 border-t border-gray-100 animate-fadeIn">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                    <span className="w-8 h-8 rounded-lg bg-blue-100 flex items-center justify-center text-blue-600 text-sm font-black">AI</span>
              ...
              <div className="bg-gray-50/80 rounded-2xl p-6 border border-gray-200 min-h-[150px] shadow-inner relative flex flex-col justify-center">
                {loading ? (
                  <div className="flex flex-col items-center justify-center py-10 text-gray-500 space-y-4">
                    <p className="animate-pulse font-medium text-blue-600">Interpretando imagens e dados clínicos...</p>
      ...
      <footer className="mt-auto py-6 border-t border-gray-200 bg-white">
        <div className="max-w-4xl mx-auto px-4 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex gap-4">
             <button onClick={() => openFooterModal('terms')} className="text-xs text-gray-500 hover:text-blue-600 transition">Termos de Uso</button>
             <button onClick={() => openFooterModal('privacy')} className="text-xs text-gray-500 hover:text-blue-600 transition">Privacidade</button>
          </div>
        </div>
      </footer>
